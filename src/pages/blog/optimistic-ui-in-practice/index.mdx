...
This approach seems pretty simple, and most of the time it gets the job done. But what if we use the `language` field elsewhere? How we would share the temporary optimistic state outside `LanguageSelect`? Like, for example, showing a user's language flag.
